...
void ntt_test()
{
	...
	Type M31 = (1L << 31) - 1;
	// primitive root = 10 + 9i;
	// primitive n-th root = (10 + 9i)^t; t = 2^(32 - n)*(2^30 - 1);
	ZNTT<32, Type> ntt(M31, 10, 9);
...
void convolution_test1()
{
	...
	Type vu[N+M]; int n;
	Type M31 = (1L << 31) - 1;
	ZNTT<32, Type>* ntt = new ZNTT<32, Type>(M31, 10, 9);
	NTT_TABLE[2] = ntt;
...
void convolution_test2()
{
	...
	Type V30 = (3 << 30) + 1, p = 5;
	// primitive root = 13;
	// primitivr n-th root = 13^t; t = (V30 - 1) / 2^n;
	NTT<30, Type>* ntt = new NTT<30, Type>(V30, p);
	NTT_TABLE[1] = ntt;
	
	n = ntt->convolution(vu, v, N, u, M);
//	vu = [7047, 9936, 11994, 15395, 7745, 5593, 1960];
	assert(n == N + M - 1);
	int i;
	for (i = 0; i < n; ++i) {
		Type w = 0;
...
{
	Type M19 = (1L << 19) - 1;
	NTT_TABLE[0] = new ZNTT<20, Type>(M19, 4, 1);

	const int N = 13, M = 4;
	Type x[N] = {0x0,0xf0000000,0xffffffff,0xffffffff,0xffffffff,0xffff,0x0,0xfe000000,0xffffffff,0xffffffff,0xffffffff,0xffffffff,0x3fff};
	Type y[M] = {0xfffc0000,0x3fffffff,0xffffffff,0xffffffff};
	Type z[N+M];
...
void M31_test()
{
	...
	for (Type wc = 2; wc <= M; ++wc) {
		for (Type ws = (wc % 2) ? 2 : 1; ws < wc; ws += 2) {
			powz<Type>(zr[0], zi[0], wc, ws, (M31 - 1), M31);
			for (int i = 1; i < N; ++i) {
				zr[i] = zr[i-1], zi[i] = zi[i-1];
				mulz<Type>(zr[i], zi[i], zr[i-1], zi[i-1], M31);
			}
			if (zr[N-1] == 1 && zi[N-1] == 0)
				if (zr[N-2] == (M31 - 1) && zi[N-2] == 0)
					if (zr[N-3] == 0 && zi[N-3] == (M31 - 1)) {
						for (int i = 0; i < N; ++i)
							mr[i] = zr[i], mi[i] = zi[i];
						printf("(%u, %u)", wc, ws);
						if (ws < M-1) printf(",");
	...
	if (L > 0) for (int i = 0; i < N; ++i) {
		Type norm = addm<Type>(mulm<Type>(mr[i], mr[i], M31), mulm<Type>(mi[i], mi[i], M31), M31);
...
void M19_test()
{
	...
	for (Type wc = 2; wc <= M; ++wc) {
		for (Type ws = (wc % 2) ? 2 : 1; ws < wc; ws += 2) {
			powz<Type>(zr[0], zi[0], wc, ws, (M19 - 1), M19);
			for (int i = 1; i < N; ++i) {
				zr[i] = zr[i-1], zi[i] = zi[i-1];
				mulz<Type>(zr[i], zi[i], zr[i-1], zi[i-1], M19);
			}
			if (zr[N-1] == 1 && zi[N-1] == 0)
				if (zr[N-2] == (M19 - 1) && zi[N-2] == 0)
					if (zr[N-3] == 0 && zi[N-3] == (M19 - 1)) {
						for (int i = 0; i < N; ++i)
							mr[i] = zr[i], mi[i] = zi[i];
						printf("(%u, %u)", wc, ws);
						if (ws < M-1) printf(",");
	...
	if (L > 0) for (int i = 0; i < N; ++i) {
		Type norm = addm<Type>(mulm<Type>(mr[i], mr[i], M19), mulm<Type>(mi[i], mi[i], M19), M19);
		printf("Z(%d) = |(%u, %u)| = %u\n", N - 1 - i, mr[i], mi[i], norm);
	}
}
